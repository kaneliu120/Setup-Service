...
export class BookingsService {
  ...
  async create(dto: CreateBookingDto) {
    const booking = this.bookingRepository.create({
      ...dto,
      appointment_time: dto.appointment_time ? new Date(dto.appointment_time) : new Date(),
    });
  ...
  private async writeToObsidian(booking: Booking) {
    // Skip Obsidian write in production unless OBSIDIAN_VAULT_PATH is explicitly set
...
`;
    ...
    // Try to open in Obsidian via URI scheme (non-blocking, cross-platform)
    if (process.env.OBSIDIAN_OPEN !== 'false') {
      const obsidianUri = `obsidian://open?vault=Obsidian%20Vault&file=${encodeURIComponent('记录/' + fileName.replace('.md', ''))}`;
      const platform = process.platform;
      const openCmd = platform === 'darwin' ? 'open' : platform === 'win32' ? 'start' : 'xdg-open';
      try {
        execFile(openCmd, [obsidianUri], () => { /* silent */ });
      } catch {
        // Ignore on servers without desktop environment
      }
    }
  }
  ...
  private async writeConsultationToObsidian(booking: Booking) {
    // Skip Obsidian write unless OBSIDIAN_VAULT_PATH is explicitly set
    const vaultPath = process.env.OBSIDIAN_VAULT_PATH;
    if (!vaultPath) {
      return;
    }
